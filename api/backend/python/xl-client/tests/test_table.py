...
@pytest.mark.asyncio
async def test_text_is_not_deleted_with_overrides():
    sheet = await create_sheet(
        "table A\n  [a] = 1\n# comment 1\noverride\nrow,[a]\n1,3\n"
    )
    table = sheet.get_table("A")
    table.overrides = None
...
@pytest.mark.asyncio
async def test_swap_fields():
    sheet = await create_sheet("table A\n  [a] = 1\n  [b] = 2\n  [c] = 3\n")
    table = sheet.get_table("A")
    table.swap_fields("a", "c")

    assert list(table.field_names) == ["c", "b", "a"]
    assert sheet.to_dsl() == "table A\n  [c] = 3\n  [b] = 2\n  [a] = 1\n"
