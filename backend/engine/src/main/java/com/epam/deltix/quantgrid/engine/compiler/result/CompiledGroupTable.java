...
public class CompiledGroupTable extends CompiledAbstractTable {
    ...
    public CompiledGroupTable(Plan plan, LinkedHashMap<String, ColumnFormat> columns) {
        this(plan, columns, REF_NA, List.of(), true);
    }

    private CompiledGroupTable(Plan plan, LinkedHashMap<String, ColumnFormat> columns,
    ...
    public CompiledTable with(Plan plan, List<FieldKey> dimensions, int currentRef, int queryRef, boolean nested) {
        ...
        return new CompiledGroupTable(plan, columns, currentRef, dimensions, nested);
    }
}