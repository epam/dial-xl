...
class CompileGroupBy {
    ...
    private CompiledGroupTable compile(CompileContext context, List<CompiledColumn> rows,
        ...
        nameValues(context, types, formats, columns);
        return new CompiledGroupTable(result, columns);
    }
    ...
    private void nameValues(CompileContext context,
                            List<AggregateType> types, List<ColumnFormat> formats,
                            LinkedHashMap<String, ColumnFormat> columns) {
        if (types.isEmpty()) {
            return;
        }

        Formula formula = context.argument(1);
        int argStart = 0;
        ...
        for (int i = 0; i < types.size(); i++) {
            ...
            if (formula instanceof FieldReference field) {
                args = List.of(field.field());
            } else if (formula instanceof FieldsReference fields) {
                List<String> names = fields.fields();
                args = argStart < names.size() ? names.subList(argStart, argEnd) : names; // repeated for autofilled
            }

            String suffix = ""; //args.isEmpty() ? "" : ("(" + String.join(", ", args) + ")");
            String name = type.name() + suffix;

            for (int j = 1; columns.containsKey(name); j++) {
                name = type.name() + j + suffix;
            }
