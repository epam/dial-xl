...
class ObservableObserver(Observer, ObservableNode):
    ...
    def _set_indexed_node(
        self,
        value: ObservableNode,
        collection: list[ObservableNode],
        index: int | None,
    ) -> int | None:
        if value is not None:
            value._attach(self)
            if index is not None:
                collection[index]._detach()
...
def notify_observer(func):
    ...
    def wrapper(self: ObservableNode, *args, **kwargs):
        ...
        if self._observer:
            ...
            event = Event(method_name=func.__name__, kwargs=bound_args.arguments)
            self._observer._notify_before(event)
