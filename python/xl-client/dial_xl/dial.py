...
async def _get_bucket(dial_base_url: str, credentials: CredentialProvider) -> str:
    async with aiohttp.ClientSession() as session:
        headers = await _auth_header(credentials)
        async with session.get(
...
def _sheets_to_project(sheets: dict[str, str]) -> str:
    output = io.StringIO()
    _YAML.dump({name: _DslStr(content) for name, content in sheets.items()}, output)
...
def _project_to_sheets(text: str) -> dict[str, str]:
    ...
    return {
        name: content for name, content in project.items() if not name.startswith("/")
    }
