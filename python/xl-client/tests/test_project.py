...
async def test_compile():
    ...
    assert_type_equality(
        b.field_type,
        PrimitiveFieldType(hash="", name="DOUBLE", is_nested=True),
    )
    assert_type_equality(
        c.field_type,
        TableFieldType(hash="", name="TABLE_REFERENCE", table_name="A", is_nested=True),
    )
    assert_type_equality(
        total_a.field_type,
        PrimitiveFieldType(hash="", name="DOUBLE", is_nested=False),
...
async def test_calculate():
    ...
    assert_type_equality(
        a.field_type, PrimitiveFieldType(hash="", name="DOUBLE", is_nested=False)
    )
    assert_type_equality(
        b.field_type, PrimitiveFieldType(hash="", name="DOUBLE", is_nested=True)
    )
    assert_type_equality(
        c.field_type,
        TableFieldType(hash="", name="TABLE_REFERENCE", table_name="A", is_nested=True),
    ...
    assert_type_equality(
        total_a.field_type, PrimitiveFieldType(hash="", name="DOUBLE", is_nested=False)
    )
    ...
    assert total_a.field_data == FieldData(
        values=["6"], start_row=0, end_row=1, total_rows=1
    )
