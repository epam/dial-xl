...
async def test_employees_count(employees_project: FrameProject) -> None:
...
async def test_lead_employees_count(employees_project: FrameProject) -> None:
...
async def test_employees_experience(employees_project: FrameProject) -> None:
...
async def test_no_assig_employees(employees_project: FrameProject) -> None:
...
async def test_no_assig_seniority_employees(employees_project: FrameProject) -> None:
...
async def test_full_name_employees(employees_project: FrameProject) -> None:
    answer = await employees_project.query(
        "Add a column that will contain full name of employee to Employees table."
    )

    def validate(_: Project, __: Sheet, table: Table, ___: Field) -> None:
...
async def test_assignment_cost(employees_project: FrameProject) -> None:
    answer = await employees_project.query(
...
async def test_most_expensive_project(employees_project: FrameProject) -> None:
    answer = await employees_project.query(
        "Please add another column to Employees table. "
        "Call it Most Expensive Project Name. "
        "Please populate it accordingly."
    )

    def validate(_: Project, __: Sheet, table: Table, ___: Field) -> None:
        assert is_table_code_regex(table, "(?i).*(SORT|MAX|MAXBY).*")

    def _get_true_costs(empty_value: str = "") -> list[str]:
        return (
            [
                "E-commerce Platform",
                "UI/UX Prototyping",
...
async def test_specific_employee(employees_project: FrameProject) -> None:
...
async def test_busy_employee(employees_project: FrameProject) -> None:
    question = "Provide me top-3 busiest employees names."
    gt_answer = (
        "Most busy employees are Joseph (optional: 3 assignments), "
        "Jane (optional: 2 assignments) and Emily (optional: 2 assignments)."
    )
...
async def test_avg_cost(employees_project: FrameProject) -> None:
    question = (
        "Calculate average assignment cost for the employee with the most assignments."
    )

    gt_answer = (
        "The average assignment cost for the employee "
        "with the most assignments can be either 60 or 62.5."
    )
