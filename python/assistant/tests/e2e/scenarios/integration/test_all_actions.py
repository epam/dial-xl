...
from tests.e2e.framework.assertion_utils import assert_regex_match
...
async def test_add_column(all_actions_project: FrameProject) -> None:
    ...
    def validate(_: Project, __: Sheet, table: Table, field: Field) -> None:
        assert is_field_code_regex(table, field, ".*\\[a\\]\\s*\\+\\s*\\[b\\].*")
...
async def test_remove_column(all_actions_project: FrameProject) -> None:
    answer = await all_actions_project.query(
        """
...
async def test_change_column(all_actions_project: FrameProject) -> None:
    answer = await all_actions_project.query(
        """
    ...
    def validate(_: Project, __: Sheet, table: Table, field: Field) -> None:
        assert is_field_code_regex(table, field, ".*\\[a\\]\\s*\\-\\s*1.*")
...
async def test_rename_column(all_actions_project: FrameProject) -> None:
    answer = await all_actions_project.query(
        """
    ...
    def validate_c(_: Project, __: Sheet, table: Table, field: Field) -> None:
        assert is_field_code_regex(table, field, ".*RANGE\\(10\\).*")
        assert field.dim

    def validate_b(_: Project, __: Sheet, table: Table, field: Field) -> None:
        assert is_field_code_regex(table, field, ".*\\[c\\]\\s*\\^\\s*2.*")
        # Unfortunately bot randomly may fix or don't fix the comment.
        #   Cannot stabilize this behavior.
        # assert field.doc_string and "In quadratic dependency from c" in field.doc_string  # noqa: E501
...
async def test_add_column_note(all_actions_project: FrameProject) -> None:
    answer = await all_actions_project.query(
        """
...
async def test_remove_column_note(all_actions_project: FrameProject) -> None:
    answer = await all_actions_project.query(
        """
    ...
    def validate(_: Project, __: Sheet, table: Table, field: Field) -> None:
        assert is_field_code_regex(table, field, ".*\\[a\\]\\s*\\^\\s*2.*")
...
async def test_change_column_note(all_actions_project: FrameProject) -> None:
    answer = await all_actions_project.query(
        """
...
async def test_add_column_decorator(all_actions_project: FrameProject) -> None:
    answer = await all_actions_project.query(
        """
...
async def test_remove_column_decorator(all_actions_project: FrameProject) -> None:
    answer = await all_actions_project.query(
        """
    ...
    def validate(_: Project, __: Sheet, table: Table, field: Field) -> None:
        ...
        assert field.doc_string
        assert "In quadratic dependency from a" in field.doc_string
...
async def test_change_column_decorator(all_actions_project: FrameProject) -> None:
    answer = await all_actions_project.query(
        """
...
async def test_change_table_decorator(all_actions_project: FrameProject) -> None:
    answer = await all_actions_project.query(
        """
...
async def test_add_table(all_actions_project: FrameProject) -> None:
    answer = await all_actions_project.query(
        """
    ...
    def validate_a(field: Field) -> bool:
    ...
    def validate_b(field: Field) -> bool:
        assert isinstance(field.field_data, FieldData)
        return field.field_data.values == [
            "1",
            "4",
    ...
    def validate(_: Project, __: Sheet, table: Table) -> None:
        assert any(validate_a(f) for f in iterate_static_fields(table))
        assert any(validate_b(f) for f in iterate_static_fields(table))
...
async def test_add_table_new_sheet(basic_project: FrameProject) -> None:
...
async def test_remove_table(all_actions_project: FrameProject) -> None:
    answer = await all_actions_project.query(
        """
        Remove table "T1"
        """
...
async def test_rename_table(all_actions_project: FrameProject) -> None:
    answer = await all_actions_project.query(
        """
    ...
    def validate(_: Project, __: Sheet, table: Table) -> None:
        ...
        assert is_field_code_regex(table, a2, ".*RANGE\\(10\\).*")
        assert not a2.doc_string
        assert not any(a2.decorators)

        b2 = get_static_field(table, "b2")
        assert b2
        assert not b2.dim
        ...
        assert is_field_code_regex(table, b2, ".*\\[a2\\]\\s*\\^\\s*2.*")
        assert b2.doc_string
        assert "In quadratic dependency from a" in b2.doc_string
...
async def test_add_manual_table(basic_project: FrameProject) -> None:
    answer = await basic_project.query(
        """
    ...
    def validate(_: Project, __: Sheet, table: Table) -> None:
        assert len(list(iterate_static_fields(table))) == 3

        assert any(d.name == "manual" for d in table.decorators)

        title = get_static_field(table, "title")
        assert title
        assert not title.dim
        title_field_group = get_field_group(table, "title")
        assert title_field_group.to_dsl() is None or is_field_code_regex(
        ...
        year = get_static_field(table, "year")
        assert year
        assert not year.dim
        year_field_group = get_field_group(table, "year")
        assert year_field_group.to_dsl() is None or is_field_code_regex(
            table, year, r"(.*\[year\]\s*\n.*)|.*NA.*"
        )
        assert isinstance(year.field_data, FieldData)
        assert len(year.field_data.values) == 10

        score = get_static_field(table, "score")
        assert score
        assert not score.dim
        score_field_group = get_field_group(table, "score")
        assert score_field_group.to_dsl() is None or is_field_code_regex(
...
async def test_add_manual_column(imdb_10_project: FrameProject) -> None:
    ...
    def validate_override(_: Project, __: Sheet, table: Table) -> None:
        field = get_static_field(table, "genre")
        assert field
        assert not field.dim
        genre_field_group = get_field_group(table, "genre")
        assert genre_field_group.to_dsl() is None or is_field_code_regex(
...
async def test_remove_manual_column(imdb_10_project: FrameProject) -> None:
    answer = await imdb_10_project.query(
        """
...
async def test_add_manual_rows(imdb_10_project: FrameProject) -> None:
    answer = await imdb_10_project.query(
        """
...
async def test_remove_manual_rows(imdb_10_project: FrameProject) -> None:
    answer = await imdb_10_project.query(
        """
