...
from tests.e2e.framework.expected_actions import AddField, AddFieldOrTable, AddTable
...
async def test_sort_by(basic_project: FrameProject) -> None:
    ...
    def validate_sortby(_: Project, __: Sheet, table: Table) -> None:
...
async def test_filter(basic_project: FrameProject) -> None:
    await basic_project.create_table(
        table_name="TSort",
        code="""
    ...
    def validate_filter(_: Project, __: Sheet, table: Table, ___: Field) -> None:
...
async def test_filter_function(imdb_simple_project: FrameProject) -> None:
...
async def test_unique_by(basic_project: FrameProject) -> None:
    await basic_project.create_table(
        table_name="TSort",
        code="""
        !placement(1,1)
    ...
    def validate_uniqueby(_: Project, __: Sheet, table: Table) -> None:
        assert is_table_code_regex(table, "(?i).*UNIQUEBY.*")
...
@pytest.mark.skip("Not included in DSL MVP subset")
async def test_find(basic_project: FrameProject) -> None:
    await basic_project.create_table(
        table_name="TFind",
        code="""
        !placement(1,1)
