...
from tests.e2e.exceptions.score_error import ScoreError
from tests.e2e.models.qg_report import QGReport
from tests.e2e.models.query_info import QueryInfo
from tests.e2e.models.stage import Attachment
...
class XLSXReport:
    ...
    def write(
        ...
        for case_name, case_reports in sorted_tests:
            ...
            self._write_summary_case(
                case_name,
                case_reports,
                stats_by_case.get(case_name),
                prior_stats_by_case.get(case_name),
            )
    ...
    @staticmethod
    def _get_avg_llm_score(reports: list[QGReport], key: str) -> dict[str, Any] | None:
        scores = [
        ...
        # -1 as score means failed score calculation,
        #   it shouldn't be included into avg calculation
    ...
    def _write_legend(self) -> None:
        ...
        for _status, _symbol, _meaning in (
            ...
            (
                "PRIOR",
                "PRIOR",
                "The PRIOR prefix indicates that the comparison is between "
                "the main commit at the time of the fork(prior commit) and the history",
            ),
        ):
    ...
    def _write_statistic(self) -> None:
        ...
        def _write_statistic_value(
            name: str, value: str, style: Format = self.default_style
        ) -> None:
        ...
        _write_statistic_value(
            f"Average llm score "
            f"({self._statistic.llm_score_runs_count}"
            f"/{self._statistic.llm_score_max_runs_count} "
            f"test cases):",
            str(llm_score_avg),
        )
        ...
        _write_statistic_value(
            f"Average redundancy score "
            f"({self._statistic.redundancy_score_runs_count}"
            f"/{self._statistic.redundancy_score_max_runs_count} "
            f"test cases):",
        ...
        _write_statistic_value(
            "Average output tokens count (generate action tokens/summary tokens):",
        ...
        if (
            ...
            pvalue_verdict = "skipped"
        _write_statistic_value(
            f"Combined p-value across all cases using Fisher's method "
            f"to detect shift in {self._statistic.global_stats.fraction_failed * 100} "
            f"% cases with alpha: {self._statistic.global_stats.alpha}, "
            f"beta: {self._statistic.global_stats.beta}, "
            f"delta: {self._statistic.global_stats.delta} "
            f"(statistical significance may fail "
            f"to be reached if the run count equals the maximum run count):",
    ...
    def _write_summary_case(  # noqa: C901
        self,
        name: str,
    ...
    def get_verdict_code(exception_name: str | None) -> str:
        match exception_name:
            ...
            case MatchError.__name__:
                return "ME"
            case ExpectedActionAssertionError.__name__:
                return "ME"
    ...
    def _write_details_sheet_test_header(
        ...
        self._summary_hyperlinks[name] = (
            f"internal:Details!{xl_rowcol_to_cell(self._details_line, 0)}"
        )
    ...
    def _write_details_report(self, run_index: int, report: QGReport) -> None:
        ...
        if report.exception_name is not None:
            ...
            if (
                report.exception_name == CompileError.__name__
                or report.exception_name == MatchError.__name__
            ):
                self.details_sheet.write(
    ...
    def _write_details_query(  # noqa: C901
        self,
        query_index: int,
        ...
        if query_info.llm_score.score is not None:
            ...
            if query_info.llm_score.score >= 0:
                score_text = (
                    f"Score: {query_info.llm_score.score}. "
                    f"Explanation: {query_info.llm_score.explanation}"
                )
            else:
        ...
        if query_info.redundancy_score.score is not None:
            ...
            if query_info.redundancy_score.score >= 0:
                score_text = (
                    f"Score: {query_info.redundancy_score.score}. "
                    f"Explanation: {query_info.redundancy_score.explanation}"
                )
            else:
                score_text = "Score was not generated."
