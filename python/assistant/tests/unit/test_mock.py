...
async def test_advertisers_total_revenue(
    xl_client: Client,
    actions_graph: CompiledStateGraph[ActionsAgentState, Context],
    mock_context: Callable[[Project, str], Context],
    shared_datadir: Path,
) -> None:
    code = (shared_datadir / "advertisers.dsl").read_text("utf-8")
...
async def test_advertisers_media_type(
    xl_client: Client,
    actions_graph: CompiledStateGraph[ActionsAgentState, Context],
    mock_context: Callable[[Project, str], Context],
) -> None:
    path = Path("tests/test_resources/advertisers.dsl")
    code = path.read_bytes().decode("utf-8")

    project = await create_project(
        xl_client, "test_advertisers_media_type", {"Main": code}
    )
    query = (
        "What are the dynamics of revenue by Media Type? "
        "Which sector is growing and which is declining?"
    )
