...
from quantgrid.startup import load_documentation_human
...
async def answer(state: GeneralState):
    response = await astream_model(
        state.config.model,
        [
            *state.config.documentation_prologue,
            load_documentation_human(state.config.question),
        ],
...
def documentation_graph() -> CompiledStateGraph:
