...
export function applyCellGraphics(
  ...
  let borderTop: PIXI.ILineStyleOptions | undefined = cellBorder;
  let borderRight: PIXI.ILineStyleOptions | undefined = cellBorder;

  // This border style is needed for top and right chart borders
  // because cells under the chart override grid lines
  if (isChartCell) {
    const chartBorder: PIXI.ILineStyleOptions = {
  ...
  const cellStyle: CellStyle = {
    ...
    border: {
      borderTop,
      borderLeft: isChartCell ? undefined : cellBorder,
      borderRight,
    },
    shadow: isChartCell
      ? undefined
