import { editor, IRange, Position, Range } from '../monaco';
...
export class CompletionFormatter {
  ...
  private _editor: editor.ITextModel;
  private _cursorPosition: Position;
  private _lineCount: number;

  constructor(editor: editor.ITextModel, position: Position) {
    this._editor = editor;
  ...
  private removeDuplicateStartOfSuggestions(): this {
    ...
    if (overlapLength > 0) {
      this._completion = completion.substring(overlapLength);
    }

    return this;
  }

  private removeDuplicateFromCurrentLineStart(): this {
    const currentLineText = this._lineText.trimStart();
    const completionText = this._completion.trimStart();

    const maxLength = Math.min(currentLineText.length, completionText.length);
    let overlapLength = 0;

    for (let length = maxLength; length > 0; length--) {
      const startOfLine = currentLineText.substring(0, length);
      const startOfCompletion = completionText.substring(0, length);

      if (startOfLine === startOfCompletion) {
        overlapLength = length;
        break;
      }
    }
    ...
    if (overlapLength > 1) {
      this._completion = this._completion.substring(overlapLength).trimStart();
    }
  ...
  private getNoTextBeforeOrAfter = () => {
    ...
    const textBeforeRange = new Range(
      0,
      0,
      this._cursorPosition.lineNumber,
      this._cursorPosition.column
  ...
  private formatCompletion = (range: IRange) => {
  ...
  public format = (
    ...
    return this.matchCompletionBrackets()
      ...
      .removeDuplicateFromCurrentLineStart()
