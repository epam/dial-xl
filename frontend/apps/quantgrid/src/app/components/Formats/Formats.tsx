...
export const Formats = () => {
  ...
  const selectedCell = useViewStore((s) =>
    isFormatDropdownOpened ? s.selectedCell : null,
  );
  ...
  const items: MenuItem[] = useMemo(() => {
    if (!isFormatDropdownOpened) return [];

    return getFormatsItems(selectedCellValue ?? '', selectedFormatExplicit);
  }, [isFormatDropdownOpened, selectedCellValue, selectedFormatExplicit]);
  ...
  useEffect(() => {
    const unsub = useViewStore.subscribe(
      (s) => s.selectedCell,
      () => {
        setIsFormatDropdownOpened(false);
        setIsDigitsModeMenuOpened(false);
      },
    );
