...
import { ChatOverlayContext, playbackLabel } from './ChatOverlayContext';
import { useChatAnsweringTabUpdate } from './useChatAnsweringTabUpdate';
...
const defaultAISheetName = 'AI answer';
...
export function ChatOverlayContextProvider({
  ...
  const { sendDialRequest, getSharedWithMeResources } = useApiRequests();
  const {
    projectSheets,
    sheetName,
    projectName,
  ...
  // Update tab according to chat generation state
  useChatAnsweringTabUpdate(isGenerating);
  ...
  const getOverlayOptions = useCallback((): ChatOverlayOptions => {
    ...
    return {
      ...
      signInOptions: {
        ...
        validationUserEmail: user?.profile.email,
      },
              ...
              selectedConversation?.isPlayback
                ...
                : {
                    buttonKey: isHidePreviewButton
                      ...
                      : viewButtonKey,
                    events: ['click'],
                    title: isHidePreviewButton ? 'Exit view' : 'View changes',
  ...
  const destroyChatOverlay = useCallback(() => {
    ...
    overlayInitialized.current = false;
