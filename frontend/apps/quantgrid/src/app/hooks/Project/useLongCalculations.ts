...
export type LongCalcRequestType = 'viewport' | 'importSync';
export type LongCalcAction = 'start' | 'end' | 'cancel';
...
export function useLongCalculations() {
  ...
  const importSyncControllers = useRef<AbortController[]>([]);
  ...
  const manageViewportRequestLifecycle = useCallback(
    (action: LongCalcAction, controller?: AbortController) => {
      switch (action) {
        case 'start':
          if (controller) {
  ...
  const manageImportSyncRequestLifecycle = useCallback(
    (action: LongCalcAction, controller?: AbortController) => {
      switch (action) {
        case 'start':
          if (controller) {
            if (!controller.signal.aborted) {
              importSyncControllers.current.push(controller);
            }
          }
          break;
        ...
        case 'end':
          if (controller) {
            importSyncControllers.current =
              importSyncControllers.current.filter((c) => c !== controller);
          }
          break;

        case 'cancel':
          importSyncControllers.current.forEach((c) => {
  ...
  const cancelAllViewportRequests = useCallback(() => {
    manageRequestLifecycle('cancel', 'viewport');
  }, [manageRequestLifecycle]);

  const cancelAllImportSyncRequests = useCallback(() => {
    manageRequestLifecycle('cancel', 'importSync');
  }, [manageRequestLifecycle]);
